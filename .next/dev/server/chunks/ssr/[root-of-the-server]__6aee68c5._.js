...
class ChunkLoader {
    ...
    async loadChunk(minx, miny, maxx, maxy) {
        const key = `${Math.round(minx)},${Math.round(miny)},${Math.round(maxx)},${Math.round(maxy)}`;
        if (this.loaded.has(key)) {
            console.log(`[ChunkLoader] Chunk ${key} already loaded`);
            return;
        }
        console.log(`[ChunkLoader] Loading chunk ${key} from server...`);
        if (!this.loaded.has(key)) console.log(`[ChunkLoader] Loading chunk ${key}`);
        const url = `http://127.0.0.1:8000/chunk?minx=${minx}&miny=${miny}&maxx=${maxx}&maxy=${maxy}&simplify=1.0`;
        //const url = "http://127.0.0.1:8000/test_chunk";
        let geojson;
        try {
            const res = await fetch(url);
            if (!res.ok) {
                console.warn(`[ChunkLoader] HTTP error for chunk ${key}: ${res.status}`);
                return;
            }
            geojson = await res.json();
            if (!geojson.features || !Array.isArray(geojson.features)) {
                console.warn(`[ChunkLoader] Invalid GeoJSON for chunk ${key}`);
                return;
            }
        } catch (err) {
            console.error(`[ChunkLoader] Failed to load chunk ${key}:`, err);
            return;
        }
        console.log(`[ChunkLoader] Received ${geojson.features.length} features`);
        if (geojson.features.length === 0) {
            console.log(`[ChunkLoader] Chunk ${key} is empty, skipping`);
            return;
        }
        const group = this.geoJSONToMesh(geojson);
        console.log(`[ChunkLoader] Created ${group.children.length} meshes for chunk ${key}`);
        group.position.set((minx + maxx) / 2, 0, (miny + maxy) / 2);
        this.scene.add(group);
        this.loaded.set(key, group);
        console.log(`[ChunkLoader] Chunk ${key} added to scene`);
    }
    geoJSONToMesh(geojson) {
        ...
        for (const f of geojson.features){
            ...
            if (geom.type === "Polygon") {
                const mesh = this.createPolygonMesh(geom.coordinates);
                group.add(mesh);
            }
            if (geom.type === "MultiPolygon") {
                for (const poly of geom.coordinates){
                    const mesh = this.createPolygonMesh(poly);
                    group.add(mesh);
                }
            }
        }
    ...
    createPolygonMesh(rings) {
        ...
        holes.forEach((holeCoords)=>{
            const hole = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$three$2f$build$2f$three$2e$core$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["Path"]();
        ...
        return new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$three$2f$build$2f$three$2e$core$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["Mesh"](geometry, material);
    }
}
}),
...
function ThreeScene({ settings }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        ...
        function toMercator(lon, lat) {
            return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$proj4$2f$lib$2f$index$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"])(wgs84, webMercator, [
                lon,
                lat
            ]);
        }
        async function updateChunks() {
            ...
            // Конвертируем координаты центра камеры в метры
            const [cx, cz] = toMercator(target.x, target.z);
            const size = 500; // размер чанка в метрах
            const minx = cx - size;
            const miny = cz - size;
            const maxx = cx + size;
            const maxy = cz + size;
            await loader.loadChunk(minx, miny, maxx, maxy);
        }
        controls.addEventListener("change", updateChunks);
        updateChunks();
        let frame;
        const loop = ()=>{
        ...
        return ()=>{
            ...
            controls.removeEventListener("change", updateChunks);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6aee68c5._.js.map