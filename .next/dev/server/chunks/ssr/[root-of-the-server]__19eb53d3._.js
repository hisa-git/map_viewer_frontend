...
class ChunkLoader {
    ...
    async loadChunk(minx, miny, maxx, maxy) {
        ...
        const url = `/chunk?minx=${minx}&miny=${miny}&maxx=${maxx}&maxy=${maxy}&simplify=1.0`;
...
function ThreeScene({ settings }) {
    ...
    useEffect(()=>{
        ...
        function updateChunks() {
            ...
            const size = 500;
            const minx = target.x - size;
            const miny = target.z - size;
            const maxx = target.x + size;
            const maxy = target.z + size;
            loader.loadChunk(minx, miny, maxx, maxy);
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__19eb53d3._.js.map